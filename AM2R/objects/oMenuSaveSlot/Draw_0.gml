...
//    if (setanks == 10) draw_sprite_ext(sGUIETank, 1, x + etankxoff + 24, y + 10, 1, 1, 0, fontcolor, 1);
    ...
    draw_set_alpha(1);
    draw_set_font(fontGUI2);
    draw_text(x + 192, y + 6, string_hash_to_newline(to_string_lz(smonsters)));
    draw_set_alpha(1);
    draw_set_font(fontMenuSmall);
    draw_set_color(fontcolor);
    draw_text(x + titleoffset, y - 9, string_hash_to_newline(title));
    draw_set_halign(fa_right);
    draw_text(x + 186, y - 9, string_hash_to_newline(difftext));
    draw_set_halign(fa_left);
} else {
    draw_set_alpha(1);
    draw_set_font(fontGUI2);
    ...
    draw_text(x + tioffset + 1, y + 4, string_hash_to_newline(newgamestr));
    draw_set_color(fontcolor);
    draw_text(x + tioffset, y + 3, string_hash_to_newline(newgamestr));
    draw_set_halign(fa_left);
} // if (saveexists) else
