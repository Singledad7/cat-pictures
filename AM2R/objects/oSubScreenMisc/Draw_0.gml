...
draw_text(x - 20, y + 15 + oControl.subScrItemOffset, string_hash_to_newline(morph));
if (global.item[2]) {
    draw_sprite(sSubScrButton, global.spiderball, x - 28, y + 25);
    draw_text(x - 20, y + 24 + oControl.subScrItemOffset, string_hash_to_newline(spider));
}
if (global.item[3]) {
    draw_sprite(sSubScrButton, global.jumpball, x - 28, y + 34);
    draw_text(x - 20, y + 33 + oControl.subScrItemOffset, string_hash_to_newline(spring));
}
if (global.item[0]) {
    draw_sprite(sSubScrButton, global.bomb, x - 28, y + 43);
    draw_text(x - 20, y + 42 + oControl.subScrItemOffset, string_hash_to_newline(bomb));
}
if (global.item[1]) {
    draw_sprite(sSubScrButton, global.powergrip, x - 28, y + 52);
    draw_text(x - 20, y + 51 + oControl.subScrItemOffset, string_hash_to_newline(pgrip));
}
if (global.item[8]) {
    draw_sprite(sSubScrButton, global.screwattack, x - 28, y + 61);
    draw_text(x - 20, y + 60 + oControl.subScrItemOffset, string_hash_to_newline(sattack));
}
if (drawlines) {
    if (global.curropt == 8) {
...
draw_text(x + 1 - 30, y + 4 - 1 + oControl.subScrHeaderOffset, string_hash_to_newline(misc));
//draw_text(x + 1 - 30, y + 4 - 45, "BOOTS");
draw_set_color(c_white);
//draw_text(x - 30, y + 3 - 1, "SUIT");
//draw_text(x - 30, y + 3 - 63, "BEAM");
draw_text(x - 30, y + 3 - 1 + oControl.subScrHeaderOffset, string_hash_to_newline(misc));
