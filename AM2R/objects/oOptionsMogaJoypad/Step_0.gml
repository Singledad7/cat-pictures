if (active) {
    ...
    if (oControl.kLeft > 0 && oControl.kLeftPushedSteps > 5 && oControl.kDown == 0 && oControl.kUp == 0) {
        if (global.curropt == 0 && global.opxjoydz > 10) {
            global.opxjoydz -= 1;
            event_user(2);
        }
    }
    if (oControl.kRight > 0 && oControl.kRightPushedSteps > 5 && oControl.kDown == 0 && oControl.kUp == 0) {
        if (global.curropt == 0 && global.opxjoydz < 100) {
            global.opxjoydz += 1;
            event_user(2);
        }
    }
    if (oControl.kMenu1 && oControl.kMenu1PushedSteps == 0 && canedit[global.curropt] && global.curropt <= 12 && global.curropt > 1 && editing == 0) {
        if (editing == 0) {
            editing = 1;
            alarm[2] = 300;
            io_clear();
    ...
    if (editing && readjoy) {
        if (check_xjoybtn() > 0 && global.curropt > 1) {
            if (global.curropt < 10 || global.curropt == 10 && global.opxjoybtn_menu1 != check_xjoybtn() && global.opxjoybtn_menu2 != check_xjoybtn()) {
                remap_duplicate_xjoybtn(check_xjoybtn());
                map_xjoybtn(global.curropt, check_xjoybtn());
            } else if (global.curropt > 10 && global.opxjoybtn_str != check_xjoybtn()) {
